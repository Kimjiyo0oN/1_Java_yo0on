...
public class arrayExample {
	...
	// 배열 응용
	public  void ex8() {
		// 인원 수를 입력받아 그 크기만큼의 정수 배열을 선언 및 할당
		// 인원 수 만큼 점수를 입력 받아
		// 합계, 평균, 최고점, 최저점을 출력
		...
		// for문에 breakpoint를 지정하면 증감식이 해석되기 전에 

		for(int i =1; i < scoreArr.length; i++) {
			if(scoreArr[i]>max) {   // 기본 최고점보다 screArr[i] 값이 더 큰 경우
				max = scoreArr[i];
			}
			if(scoreArr[i]<min) {   // 기본 최저점보다 screArr[i] 값이 더 작은 경우
				min = scoreArr[i];
			}
		}
	...
	public  void ex9() {
		...
		//입력 받은 정수가 배열에 존재하면 몇번 인덱스에 존재하는 지 출력
		//단, 없다면 "존재하지 않습니다." 출력
	...
	public void createLottoNumber() {
		...
		// 1. 1~45 사이의 중복되지 않은 수 6개 -난수로 6개 뽑기
		// 2. 생성된 난수를 오름 차순 정렬
		...
		for(int i =0; i < lotto. length; i++) {
			...
			for(int x=0; x<i ; x++) {
				if(lotto[x]==ran) {
					...
					break; //추가 검사가 불필요하기 때문에 
				}
			}
		}
		//Arrays.xort(배열명): 배열 내 값을 오름 차순으로 정렬
		//-> Java가 미리 만들어서 제공
		Arrays.sort(lotto);
	...
	public void ex10() {
		// 얕은 복사
		// -참조하는 주소만을 복사하여
		// 서로 다른 참조변수가 하나의 배열(또는 객체)를 참조하게 하는 복사 방법
...
//		System.arraycopy(arr, 0, copyArr, 0, arr.length);
	...
	public void ex12() {
		//null 의미 
		// - 참조 하는 것(배열, 객체)이 없다를 의미
		
		int arr1[] = new int[3];
		
		System.out.println(arr1 == null);
		// arr1 == null -> false == arr1 참조변수
...
//		int[] arr2;
		//배열 참조변수를 선언만 했을 때 == 저장된 값이 없다
		//The local variable arr2 may not have been initialized
		
		int[] arr2 =null;
		//배열 참조변수를 선언 및 null로 초기화 
