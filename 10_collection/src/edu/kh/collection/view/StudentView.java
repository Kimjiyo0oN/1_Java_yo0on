...
public class StudentView {
	...
	public void displayMenu() {
		...
		do {
		    ...
		    try {
				...
				//input에 잘못된 값을 넣으면 input =0 이됨
				input =-1; //첫 반복 시 잘못 입력된 경우 while문이 종료되지 않도록 하는 값
				
				sc.nextLine(); //입력 버퍼에 남아있는 잘못된 문자열을 꺼내와 제거 //앞에서 	
			}
	...
	private void selectAll() {
		...
		// 향상된 for문
		// - 배열 또는 컬렉션의 모든 요소를
		// 처음부터 끝까지 순차 접근할 때 사용하는 for문 (for each문)
	...
	private void updateStudent() {
		...
		// 인덱스가 일치하는 학생 정보를 수정하면 "실패(인덱스 불일치)" 출력
	...
	private void selectStudent() {
		...
		List<Student> resultList =service.selectSudent1(name);
	...
	private void selectStudent2() {
		...
		List<Student> resultList =service.selectSudent2(name);
